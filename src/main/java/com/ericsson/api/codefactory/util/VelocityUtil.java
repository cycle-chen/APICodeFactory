...
public class VelocityUtil {
	...
	private static VelocityProxy vc = null;
	
	private static volatile boolean isInit = false;
	
	public  static synchronized VelocityProxy getVelocityProxy(VModel m){
		if( m == VModel.SINGLETON){
			if( vc == null ){
				vc = new VelocityProxySingle();
			}
		}else{
			if( vc == null ){
				//vc = new VelocityProxyPrototype();
			}
		}
		return vc;
	}
	...
	public static synchronized VelocityProxy getDefaultInitVelocity(){
		VelocityProxy vp = VelocityUtil.getVelocityProxy(VModel.SINGLETON);
		if( !isInit ){
			vp.setProperty(RuntimeConstants.RESOURCE_LOADER,"file,classpath");
			vp.setProperty("classpath.resource.loader.class", "org.apache.velocity.runtime.resource.loader.ClasspathResourceLoader");
			vp.setProperty("file.resource.loader.cache", true);//缓存模板文件
			Properties rbfp = new Properties();
			InputStream in = ClassUtil.getDefaultClassLoader().getResourceAsStream("rbfvelocity.properties");
			if( in == null ){
				in = ClassUtil.getDefaultClassLoader().getResourceAsStream("com/ericsson/api.codefactory/util/rbfvelocity.properties");
			}
			try {
				rbfp.load(in);
			} catch (IOException e) {
				throw new RuntimeException("无法配置Velocity 模板路径",e);
			}
			String vmFilePath = rbfp.getProperty("vmfilepath");
			vp.setProperty(RuntimeConstants.FILE_RESOURCE_LOADER_PATH, ClassUtil.getDefaultClassLoader().getResource("").getPath()+"/"+vmFilePath.replaceAll("//", File.separator));
			vp.setProperty(RuntimeConstants.FILE_RESOURCE_LOADER_CACHE, true);
			vp.init();
			isInit = true;
		}
		return vp;
	 ...
	 *
	 */
	public interface VelocityProxy{
		 public void init();
	...
	static class VelocityProxySingle implements VelocityProxy{
		...
		public boolean evaluate(Context context, Writer writer, String logTag,
				InputStream instream) throws ParseErrorException,
				MethodInvocationException, ResourceNotFoundException,
				IOException {
		...
		public boolean evaluate(Context context, Writer writer, String logTag,
				Reader reader) throws ParseErrorException,
				MethodInvocationException, ResourceNotFoundException {
		...
		public boolean invokeVelocimacro(String vmName, String logTag,
			...
			return Velocity.invokeVelocimacro(vmName, logTag, params, context, writer);
		}

		public boolean mergeTemplate(String templateName, Context context,
				Writer writer) throws ResourceNotFoundException,
				ParseErrorException, MethodInvocationException {
			return Velocity.mergeTemplate(templateName, context, writer);
		}
		...
		public boolean mergeTemplate(String templateName, String encoding,
			...
			return Velocity.mergeTemplate(templateName, encoding, context, writer);
		}
		...
		public boolean templateExists(String resourceName) {
			return Velocity.templateExists(resourceName);
		}
		...
		public void setApplicationAttribute(Object key, Object value) {
			Velocity.setApplicationAttribute(key, value);
		}

		public Object getApplicationAttribute(Object key) {
			return RuntimeSingleton.getApplicationAttribute(key);
		}
